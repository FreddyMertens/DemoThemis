...
Object.keys(rewrites).forEach(id => {
  // Look for the object definition. We know it has id: "something" and later copy: "something"
  // It could have newlines or be on the same line.
  // We can use a regex that safely matches the copy attribute inside the same object block.
  
  // Since we know the previous copy string was wrapped in double quotes, we can do this:
  const regex = new RegExp(`(id:\\s*"${id}"[\\s\\S]*?copy:\\s*")[^"]+(".*})`);
