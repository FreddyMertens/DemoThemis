...
function checkChapterSequence(failures) {
  ...
  const underhoodMap = (demoThemis.match(/<div\b[^>]*class=["'][^"']*\bunderhood-map\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i) || ["", ""])[1];
  assert((underhoodMap.match(/class=["']link-cue["']/g) || []).length === 3, "DemoThemis under-the-hood links must expose a clear button cue", failures);
  assert(!/DemoThemis turns rented arbitration|Verified humans, live reserves|Completed cases, difficulty-adjusted scoring/i.test(underhoodMap), "DemoThemis under-the-hood links must not include the old blurbs", failures);
